class CartPage {
  ...
  removeAllCartItems(itemCounts) {
    Array.from({ length: itemCounts }).forEach((_, ind) => {
      // Get the first item and click the delete button
      cy.log(`Item${ind}`);

      this.elements.cartItems().eq(0).find(this.elements.deleteBtn).click();
      cy.reload();
      cy.wait(500); // Adjust or remove based on your application’s behavior
    });
    return;
  }
}
